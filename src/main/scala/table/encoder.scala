...
object TextTableEncoder:
  ...
  class DerivedTextTableEncoder[A](f: A => List[Any], labels: List[String], textEncoders: List[TextEncoder[Any]], aligns: List[Align[Any]])
    extends TextTableEncoder[A]:
    def encode(xs: Iterable[A]): TextTable = 
      val headerRow = labels.map(label => TextTable.Cell(label, Alignment.Left))
      val dataRows = xs.map: x =>
        f(x).zip(textEncoders).zip(aligns).map:
          case ((value, encoder), align) =>
            TextTable.Cell(encoder.encode(value), align.alignment)
  ...
  inline def derived[P <: Product](using m: Mirror.ProductOf[P]): TextTableEncoder[P] =
    ...
    val alignments = summonAll[Tuple.Map[m.MirroredElemTypes, Align]].toList.asInstanceOf[List[Align[Any]]]
